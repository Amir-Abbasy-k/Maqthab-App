...
function Message(props) {
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={{flex: 1}} onPress={()=> navigation.navigate('Chat', {name:item.sender?.name, chatListId: item.id, chatId: {me: item.sender.id, reci: item.receiver_id}})}>
        <Text style={{fontSize: 16, fontWeight: 'bold'}}>{item.sender?.name}</Text>
