...
import APIKit from '../../../src_student/services/ApiKit';
...
const Home = props => {
  ...
  const _getTeachersList = () => {
    ...
    APIKit.post('/student/teacher/search', body).then(res => {
      if (res.status == 200) {
  ...
  const _getFilterData = () => {
    APIKit.get('/student/teacher/search').then(data => {
      const list = [];
        ...
        {tutors.length != 0 ? (
          tutors.map((item, key) => (
            <ProfileCard
              data={item}
              key={Math.random()}
              onPress={() => props.navigation.navigate('Profile', {tutorId: item.id })}
              onPressChat={() =>
                props.navigation.navigate('Chat', {name: item.name, chatId: {me: me?.id, reci: item?.id}})
              }
            />
          ))
